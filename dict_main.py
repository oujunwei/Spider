...
posdict = wf.readFile(path+"\\emotion_dict\\pos_all_dict.txt",'r','utf-8')
negdict = wf.readFile(path+"\\emotion_dict\\neg_all_dict.txt",'r','utf-8')

# 程度副词词典
mostdict = wf.readFile(path+'\\degree_dict\\most.txt','r','utf-8')   # 权值为2
verydict = wf.readFile(path+'\\degree_dict\\very.txt','r','utf-8')   # 权值为1.5
moredict = wf.readFile(path+'\\degree_dict\\more.txt','r','utf-8')   # 权值为1.25
ishdict =  wf.readFile(path+'\\degree_dict\\ish.txt','r','utf-8')   # 权值为0.5
insufficientdict = wf.readFile(path+'\\degree_dict\\insufficiently.txt','r','utf-8')  # 权值为0.25
inversedict = wf.readFile(path+'\\degree_dict\\inverse.txt','r','utf-8')  # 权值为-1
...
def handel_result(results):
	...
	text_pos_number = "积极条数为 " + str(pos_number) + " 条，占全部比例的 %" + str(pos_number_ratio*100)
	text_neg_number = "消极条数为 " + str(neg_number) + " 条，占全部比例的 %" + str(neg_number_ratio*100)
	text_mid_number = "中性情感条数为 " + str(mid_number) + " 条，占全部比例的 %" + str(mid_number_ratio*100)
	##################################正负极性平均得分统计
...
if __name__ == '__main__':
	...
	# wf.save(path+"\\emotion_dict\\test_result.txt",results)
